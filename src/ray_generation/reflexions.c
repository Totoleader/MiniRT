		...
		int *refl)
{
	...
	if (closest_hit.obj)
	{
		...
		new_shading.color = shading_color(&closest_hit, &new_shading, refl, vec_scale(ray.d, -1));

		shade->color = color_add(shade->color, color_scale(new_shading.color,mult * new_shading.intensity / 255.0f));
	}
}
